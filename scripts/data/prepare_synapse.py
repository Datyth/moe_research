...
from scripts.data.synapse_conversion import (
    HU_WINDOW,
    SYNAPSE_LABELS,
    convert_cases,
    write_dataset_json,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs=2,
        metavar=("MIN", "MAX"),
        default=list(HU_WINDOW),
        help=(
            "HU range mapped to [0, 1]. Defaults to the BTCV window the MoE-SAM "
            "authors confirmed by email, [-150, 500]. Pass -125 275 to build the "
            "8-organ Synapse benchmark instead."
        ),
    )
...
def main() -> None:
    ...
    print(f"HU window: [{hu_window[0]:g}, {hu_window[1]:g}] -> [0, 1]")

    training_records = convert_cases(
        cases=train_cases, output_root=data_root, split_name="train", hu_window=hu_window
    )
    validation_records = convert_cases(
        cases=val_cases, output_root=data_root, split_name="val", hu_window=hu_window
    )
    test_records = convert_cases(
        cases=test_cases, output_root=data_root, split_name="test", hu_window=hu_window
    )

    manifest = {
        "labels": SYNAPSE_LABELS,
        "version": "synapse-btcv-v3",
        "split_metadata": {
            "seed": args.seed,
            "val_ratio": args.val_ratio,
            "test_ratio": args.test_ratio,

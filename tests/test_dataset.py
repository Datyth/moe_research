...
from scripts.data.synapse_conversion import HU_WINDOW as BTCV_HU_WINDOW
...
class TestAMOS22Conversion(unittest.TestCase):
    ...
    def test_window_and_normalize_honours_a_dataset_specific_window(self):
        # BTCV maps [-150, 500] rather than the AMOS22/Synapse-CT default;
        # see docs/moe_sam_author_clarifications.md.
        volume = np.array([-2000.0, -150.0, 175.0, 500.0, 3000.0])

        normalized = window_and_normalize(volume, hu_window=BTCV_HU_WINDOW)

        self.assertEqual(BTCV_HU_WINDOW, (-150.0, 500.0))
        self.assertTrue(np.allclose(normalized, [0.0, 0.0, 0.5, 1.0, 1.0]))

    def test_window_and_normalize_rejects_a_degenerate_window(self):
        volume = np.array([0.0, 1.0])

        with self.assertRaisesRegex(ValueError, "max > min"):
            window_and_normalize(volume, hu_window=(275.0, -125.0))

...
class Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-iteration schedulers (e.g. WarmupPolyLR, which reproduces
        # MoE-SAM's warmup + polynomial decay) are stepped inside the batch
        # loop; epoch schedulers are stepped once per epoch.
        ...
        for epoch in range(self.start_epoch, self.config.epochs + 1):
            ...
            if self._should_stop_early():
                print(
                    ...
                    f"{self.config.monitor!r} did not improve for "
    ...
    def resume(self, checkpoint_path: Path) -> int:
        ...
        # Checkpoints written before early stopping existed carry no streak;
        # restarting them from zero is the only sound default.
    ...
    def _step_scheduler(self, validation_metrics: dict[str, float] | None) -> None:
        if self.scheduler is None or self.step_scheduler_per_iteration:
            # Per-iteration schedules (WarmupPolyLR) already advanced once per
            # batch inside `_train_epoch`; stepping again here would consume the
            # schedule `len(train_loader)` times too fast.
            return
    ...
    def _train_epoch(self, epoch: int) -> tuple[float, dict[str, float]]:
        ...
        for batch_number, batch in enumerate(self.train_loader, start=1):
            ...
            self.scaler.update()
            if self.scheduler is not None and self.step_scheduler_per_iteration:
                self.scheduler.step()
    ...
    def _save_checkpoint(
        ...
        *,
        ...
        checkpoint = {
            ...
            # Persisted so `resume` continues an in-progress patience streak
            # instead of silently granting the run a fresh set of epochs.
